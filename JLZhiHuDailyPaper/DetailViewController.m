...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([str hasPrefix:@"myweb:imageClick:"]) {
        ...
        WebViewController *webVC = [[WebViewController alloc]initWithString:str];
...
//计算frame
- (void)calculateFramewithDetailStory:(DetailStory *)detailStoy {
    
    [self.webView loadHTMLString:self.detailStory.htmlUrl baseURL:nil];
    
    if (detailStoy.image) {
        [self.view addSubview:self.topView];
        self.topView.detailStory = detailStoy;
    }else {
        [UIApplication sharedApplication].statusBarStyle = UIStatusBarStyleDefault;
    }
    
    //第一个有图
    if ([self.tool isTheFirstNewsWithStoryId:self.storyId] && detailStoy.image) {
        self.firstLabel.y = 20;
        [self.topView addSubview:self.firstLabel];
        
        //第一个没图
    }else if ([self.tool isTheFirstNewsWithStoryId:self.storyId] && !detailStoy.image) {
...
- (TopView *)topView {
    ...
    if (_topView == nil) {
        _topView = [TopView attchToView:self.view observeScrollView:self.webView];
    }
    return _topView;
}
...
- (void)setStoryId:(NSNumber *)storyId {
    ...
    _storyId = storyId;
    [DetailViewTool getDetailStoryId:storyId Callback:^(id obj) {
